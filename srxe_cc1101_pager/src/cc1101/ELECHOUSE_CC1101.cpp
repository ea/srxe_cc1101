...
### additionally modified radio configuration and pins for SRXE by ea
   ...
   // SpiWriteReg(CC1101_FSCTRL0,  0x00);
    ...
    switch(f)
    {
	  ...
	  case F_433:
        ...
        break;
      case F_DAP:
        SpiWriteReg(CC1101_FREQ2,    F2_DAP);
        SpiWriteReg(CC1101_FREQ1,    F1_DAP);
        SpiWriteReg(CC1101_FREQ0,    F0_DAP);
        break;        
	  default: // F must be set
	  	break;
	}
	
    //same config as Goodwatch POCSAG app
    SpiWriteReg(CC1101_MDMCFG4, 0xF5);
    SpiWriteReg(CC1101_MDMCFG3, 0x83);
    SpiWriteReg(CC1101_MDMCFG2, 0x82);
    SpiWriteReg(CC1101_MDMCFG1, 0x72);
    SpiWriteReg(CC1101_DEVIATN, 0x31);
    ...
    SpiWriteReg(CC1101_PKTLEN,    0);

    SpiWriteReg(CC1101_SYNC1,   0x83);
    SpiWriteReg(CC1101_SYNC0,   0x2d);
    SpiWriteReg(CC1101_ADDR,    0xea);
    ...
    SpiWriteReg(CC1101_MCSM0,   0x1C);

    SpiWriteReg(CC1101_IOCFG2,  0x22);
    SpiWriteReg(CC1101_IOCFG0,  0x06);
    SpiWriteReg(CC1101_FIFOTHR, 0x44);
    SpiWriteReg(CC1101_PKTCTRL1,0x01);

    SpiWriteReg(CC1101_FREQ2,  0x10);
    SpiWriteReg(CC1101_FREQ1,  0xec);
    SpiWriteReg(CC1101_FREQ0,  0x81);
}
...
byte ELECHOUSE_CC1101::ReceiveData(byte *rxBuffer)
{
    ...
    byte *tmp = rxBuffer;
    byte rxbytes;
    while((rxbytes = SpiReadStatus(CC1101_RXBYTES) & BYTES_IN_RXFIFO ))
	{

		//SpiReadReg(CC1101_RXFIFO);
		SpiReadBurstReg(CC1101_RXFIFO,tmp,rxbytes<0x40?rxbytes:0x40);
		//SpiReadBurstReg(CC1101_RXFIFO,status,2);
		tmp+=rxbytes<0x40?rxbytes:0x40;
        size+=rxbytes<0x40?rxbytes:0x40;
	}
